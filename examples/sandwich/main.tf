...
locals {
  external_subnet = distinct([for subnet in var.external_subnet_ids : subnet[0].subnet_id if subnet[0].subnet_id != null])[0]
  internal_subnet = distinct([for subnet in var.internal_subnet_ids : subnet[0].subnet_id if subnet[0].subnet_id != null])[0]
...
resource "google_compute_address" "int" {
  project      = var.project_id
  name         = format("%s-int", var.prefix)
  description  = "Static IP address for BIG-IP private ingress via ILB"
...
module "nlb" {
  source                 = "github.com/f5devcentral/terraform-google-f5-bigip-ha//modules/ilb?ref=v1.0.0"
  prefix                 = format("%s-nlb", var.prefix)
  project_id             = var.project_id
...
module "ilb" {
  source                 = "github.com/f5devcentral/terraform-google-f5-bigip-ha//modules/ilb?ref=v1.0.0"
  prefix                 = format("%s-ilb", var.prefix)
  project_id             = var.project_id
  address                = google_compute_address.int.address
  region                 = local.region
  instance_groups        = module.ha.target_groups
  labels                 = var.labels
  target_service_account = var.service_account
