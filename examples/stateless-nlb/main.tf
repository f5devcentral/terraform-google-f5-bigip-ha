...
module "template" {
  source          = "git::https://github.com/f5devcentral/terraform-google-f5-bigip-ha//modules/template?ref=v0.2.3"
  project_id      = var.project_id
  prefix          = var.name
  description     = <<-EOD
...
module "bigip_ha" {
  source            = "git::https://github.com/f5devcentral/terraform-google-f5-bigip-ha//modules/stateless?ref=v0.2.3"
  project_id        = var.project_id
  prefix            = var.name
  description       = <<-EOD
