...
variable "protocols" {
  ...
  description = <<-EOD
The IP protocols that will be enabled in the network load balancer.
...
variable "target_service_account" {
  type = string
  validation {
    condition     = coalesce(var.target_service_account, "unspecified") == "unspecified" || can(regex("^(?:[a-z][a-z0-9-]{4,28}[a-z0-9]@[a-z][a-z0-9-]{4,28}[a-z0-9]\\.iam|[0-9]+-compute@developer)\\.gserviceaccount\\.com$", var.target_service_account))
...
variable "subnet" {
  ...
  description = <<-EOD
The fully-qualified subnetwork self-link that the health check firewall rule will
be applied to.
EOD
}
